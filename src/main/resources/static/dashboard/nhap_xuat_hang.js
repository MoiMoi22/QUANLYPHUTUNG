...
confirmDelete.addEventListener("click", () => {
    ...
    const deleteUrl = `http://localhost:8080/warehouseStaff/${prefix}/delete/${id}`;
    const getAllUrl = `http://localhost:8080/warehouseStaff/${prefix}/all/`;
...
function renderTable(tableId, users) {
    ...
    if (tableId.includes("order")) {
		            ...
		            <td>
	                    <ul class="action-list list-inline">
	                        <li class="list-inline-item">
	                            <a href="/warehouseStaff/order_review(orderId=1)" class="traNo" data-tip="Xuất hàng" data-id="order  1">
        ...
        users.forEach((purchaseOrder, index) => {
            tbody.innerHTML += `
		        <tr>
		            <td>${index + 1}</td>
		            ...
		            <td>
                        <ul class="action-list list-inline">
                        	<li class="list-inline-item">
				                <a th:href="@{/warehouseStaff/purchase_review(orderId=${purchaseOrder.orderId})}" class="traNo" data-tip="Xuất hàng" th:data-id ="purchase+'  '+${purchaseOrder.orderId}"><i class="fas fa-dolly"></i></a>
				            </li>			                            
				         </ul>
		            </td>
		        </tr>`;
...
function updatePage(table, newPage) {
	...
	if (searchInput.value.trim() === '') {
	    fetchDataForPage(table);
	} 
	// Mode bình thường
	else {
		fetchDataForSearch(table, searchInput.value)
...
function fetchDataForPage(table) {
    const apiUrl = `http://localhost:8080/warehouseStaff/${table}/all`;
    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
...
function fetchDataForSearch(table, input_search) {
    const apiUrl = `http://localhost:8080/warehouseStaff/${table}/search/${input_search}`;
    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
			isFinal[table] = data.isFinal;
